...
namespace Scraper
{
    class Program
    {
        static void Main()
        {
            ...
            firefox.Navigate().GoToUrl("https://cadres.apec.fr/home/mes-offres/recherche-des-offres-demploi/liste-des-offres-demploi.html?sortsType=SCORE&sortsDirection=DESCENDING&nbParPage=20&typeAffichage=detaille&page=0&lieux=711&motsCles=web");
            var source = "apec.fr";

            // find list of job offers with an Xpath parsing
            var offers = firefox.FindElementsByXPath("//div[@class = 'offre-result ng-scope']");

            // for each job offer, find title, body, date and link
            foreach (var offer in offers)
            {
                // finding title
                var title = offer.FindElement(By.XPath(".//span[@itemprop = 'title']")).Text;
                Console.WriteLine(title);

                // finding body
                var body = offer.FindElement(By.XPath(".//p[@class = 'annonce']")).Text;
                Console.WriteLine(body);

                // finding date
                var date_raw = offer.FindElement(By.XPath(".//p[@class = 'date']")).Text;
                ...
                var link = offer.FindElement(By.XPath(".//h2[@class = 'titreOffreH2']/a")).GetAttribute("href");
                Console.WriteLine(link);

                // creating an article object with all these information
                Article art = new Article { title = title, body = body, link = link, created = date, source = source };
